...
(function chefElasticScroll() {
  ...
  let velocity = 0;
  let offset = 0;
  const stiffness = 0.08;   // rigidité du ressort
  const damping = 0.82;     // amortissement
  ...
  function animate() {
    ...
    // La cible est le décalage inverse du delta de scroll, amorti
    const target = -delta * 1.4;
    velocity += (target - offset) * stiffness;
    velocity *= damping;
    offset += velocity;

    // Clamp pour ne pas que ça parte trop loin
